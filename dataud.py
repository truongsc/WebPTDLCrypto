...
def clean_data(df, token):
    ...
    # 6. Kiểm tra dữ liệu trống
    if final_rows == 0:
        print(f"Cảnh báo: Không có dữ liệu hợp lệ cho {token} ngày hôm qua.")
    
    return df

def get_klines_data(binance_symbol, token, interval='1d'):
    """Lấy dữ liệu OHLCV cho ngày hôm qua."""
    # Tính ngày hôm qua (D-1)
    ...
    if not klines:
        print(f"Không có dữ liệu cho {binance_symbol} ngày {end_time.strftime('%Y-%m-%d')}")
        return pd.DataFrame()
...
def create_table(connection):
    """Tạo bảng historical_prices nếu chưa có."""
...
def save_to_mysql(df, connection):
    ...
    try:
        ...
        connection.commit()
        print(f"Đã lưu {len(data_to_insert)} records cho {df['symbol'].iloc[0]} ngày {df['timestamp'].iloc[0]}")
    except Error as e:
...
def main():
    ...
    print(f"Updating giá ngày hôm qua cho 50 tokens: {TOP_50_TOKENS}")
    for token in TOP_50_TOKENS:
        ...
        try:
            ...
            df = get_klines_data(binance_symbol, token)
